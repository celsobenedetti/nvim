--- Tests for lib.awesome_diff.parse_items (AwesomeDiff quickfix builder)
--- Run with: luajit tests/lib/test_awesome_diff.lua
---
--- Pure functions; no vim dependency.

package.path = './lua/?.lua;' .. package.path

local awesome_diff = require('lib.awesome_diff')
...
assert_eq(awesome_diff.parse_items(sample, 42), {
...
assert_eq(awesome_diff.parse_items(binary, 7), { { bufnr = 7, lnum = 1, text = 'logo.png' } }, 'binary section')
...
assert_eq(awesome_diff.parse_items(renamed, 3), { { bufnr = 3, lnum = 1, text = 'new.txt' } }, 'pure rename')
...
assert_eq(awesome_diff.parse_items(trailing, 9), {
  { bufnr = 9, lnum = 5, text = 'a.txt' },
  { bufnr = 9, lnum = 8, text = 'data.bin' },
}, 'text file then binary tail')

describe('parse_items: degenerate inputs')
assert_eq(awesome_diff.parse_items({}, 1), {}, 'empty diff buffer')
assert_eq(awesome_diff.parse_items({ 'not a diff' }, 1), {}, 'non-diff content')
