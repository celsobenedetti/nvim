...
M.options = {
  ...
  icon_start = '🗣️', -- Icon for macro start
...
local function notify_macro_start(register)
  local message = M.format_notification(M.options.icon_start, M.options.message_start .. register)
...
local function notify_macro_end(register)
  local message = M.format_notification(M.options.icon_end, M.options.message_end .. register)
  vim.notify(message, vim.log.levels.INFO, {
    id = M.options.id,
  })
end
