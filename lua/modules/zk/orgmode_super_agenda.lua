-- NOTE: this may not be all that useful
return {
    ...
    require("org-super-agenda").setup({
      ...
      groups = {
        ...
        },
        {
          name = "⏰ Deadlines",
          matcher = function(i)
            return i.deadline ~= nil and i.todo_state ~= "DONE" and not i:has_tag("personal")
          end,
        },
        {
          name = "⭐ Important",
          matcher = function(i)
            return i.priority == "A" and (i.deadline ~= nil or i.scheduled ~= nil)
          end,
        },
        {
          name = "⏳ Overdue",
          matcher = function(it)
            return it.todo_state ~= "DONE"
              and ((it.deadline and it.deadline:is_past()) or (it.scheduled and it.scheduled:is_past()))
          end,
        },
        {
          name = "🏠 Personal",
        ...
        },
        {
          ...
          matcher = function(it)
            ...
            local deadline_ok = it.deadline
              and it.deadline:days_from_today() >= 0
              and it.deadline:days_from_today() <= days
            local sched_ok = it.scheduled
              and it.scheduled:days_from_today() >= 0
              and it.scheduled:days_from_today() <= days
