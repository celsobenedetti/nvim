--- Diff: two-revision diff explorer on top of fugitive.
...
M.open = function(rev1, rev2)
  ...
  if #items > 0 then
    vim.fn.setqflist({}, ' ', { title = ('Diff %s..%s'):format(rev1, rev2), items = items })
  ...
  elseif result.exit_status == 0 then
    vim.cmd('tabclose')
    vim.notify('Diff: no changes between ' .. rev1 .. ' and ' .. rev2, vim.log.levels.INFO)
  else
